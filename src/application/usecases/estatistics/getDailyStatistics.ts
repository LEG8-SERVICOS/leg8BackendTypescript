...
export default async function calcularEstatisticasPorUsuarioDaily(): Promise<WorkStatistics[]> {
    const responseUsers = await axios.get<User[]>("https://leg8backendtypescript-t3md.onrender.com/users");
    const responseRecords = await axios.get<WorkRecord[]>("https://leg8backendtypescript-t3md.onrender.com/records");
    const responseAttestations = await axios.get<AttestationRecord[]>("https://leg8backendtypescript-t3md.onrender.com/attestations");
